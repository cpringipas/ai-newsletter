...
async function fetchAndStoreArticles() {
  ...
  for (const result of results) {
    if (result.status !== "fulfilled") {
      console.error("Unexpected feed fetch failure:", result.reason);
      continue;
    }

    if (result.value.error) {
      console.error(
        `Failed to fetch source ${result.value.source.name}:`,
...
async function processTopArticles(limit = config.maxStoriesPerIssue) {
  ...
  for (const article of selected) {
    if (article.status === "processed" && article.ai_title_el) {
      continue;
    }
...
async function createOrUpdateTodayIssue() {
  ...
  if (!stories.length) {
    throw new Error("No stories available to build the newsletter.");
  }
  ...
  await db.run(
    `
      INSERT INTO issues (
        issue_date, slug, title, subject_line_el, curiosity_hook_el, intro_el, tool_of_day_el, idea_of_day_el, share_cta_el, html_content, text_content, status
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft')
      ON CONFLICT(issue_date) DO UPDATE SET
        slug = excluded.slug,
        title = excluded.title,
        subject_line_el = excluded.subject_line_el,
        curiosity_hook_el = excluded.curiosity_hook_el,
        intro_el = excluded.intro_el,
        tool_of_day_el = excluded.tool_of_day_el,
        idea_of_day_el = excluded.idea_of_day_el,
        share_cta_el = excluded.share_cta_el,
  ...
  for (const [index, story] of stories.entries()) {
    await db.run(
      `INSERT INTO issue_articles (issue_id, article_id, position) VALUES (?, ?, ?)`,
      [issue.id, story.id, index + 1]
    );
  }
